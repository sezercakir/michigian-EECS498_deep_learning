...
class TwoLayerNet(object):
  ...
  def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    self.params['b1'] = torch.zeros(hidden_dims,device='cpu').to(dtype)
    self.params['b2'] = torch.zeros(num_classes,device='cpu').to(dtype)
    self.params['w1'] = torch.normal(0,weight_scale,(input_dim,hidden_dims))
    self.params['w2'] = torch.normal(0,weight_scale,(hidden_dims,num_classes))
  ...
  def loss(self, X, y=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    A, linear_ReLU_cache = Linear_ReLU.forward(X,self.params['W1'] , self.params['b1'])
    scores , linear_cache = Linear.forward(A, self.params['W2'] , self.params['b2'])
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
    ...
    d_hid, grads['W2'], grads['b2'] = Linear.backward(dout, linear_cache )
    _ , grads['W1'] , grads['b1']   = Linear_ReLU.backward(d_hid, linear_ReLU_cache)
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers):
      if i==self.num_layers-1:
        scores , linear_cache__ = Linear.forward(A, self.params['W'+str(i+1)] , self.params['b'+str(i+1)])
        linear_cache.append(linear_cache__)
        continue
      A, linear_ReLU_cache__ = Linear_ReLU.forward(temp_X,self.params['W'+str(i+1)] , self.params['b'+str(i+1)])
      if self.use_dropout:
        A, drop_out_cache__ = Dropout.forward(temp_X, self.dropout_param)
        drop_out_cache.append(drop_out_cache__)
      temp_X = A
    ...
    loss, dout = softmax_loss(scores, y)
    for i in range(self.num_layers):
      parameter_num = str(i+1)
      temp_W = self.params['W'+parameter_num]*self.params['W'+parameter_num]
      loss +=  self.reg * torch.sum(temp_W) * 0.5
   
    for i in reversed(range(self.num_layers)):
      parameter_num = str(i+1)

      if i == self.num_layers-1:
        dx, dw, db = Linear.backward(dout, linear_cache.pop() )
      else:
        if self.use_dropout:
          dx = Dropout.backward(dx, drop_out_cache[i])
        dx, dw, db = Linear_ReLU.backward(dx,linear_ReLU_cache[i])
        

      grads['W'+parameter_num] = dw + self.reg * self.params['W'+parameter_num]
      grads['b'+parameter_num] = db
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
def adam(w, dw, config=None):
  ...
  w += - config['learning_rate'] * mt / (torch.sqrt(vt) + config['epsilon'])
  next_w = w 
  #############################################################################
  #                              END OF YOUR CODE                             #
  #############################################################################
